...
def compute_distances(X1, X2):
    ...
    X1X2Product = X1 @ X2.T
    X1_dist = np.sum(X1 ** 2, axis=1, keepdims=True)
    X2_dist = np.sum(X2 ** 2, axis=1, keepdims=True).T

    dists = -2 * X1X2Product + X1_dist + X2_dist 
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        closest_y = y_train[np.argsort(dists[i])[:k]].tolist()
        count = np.zeros((16, ))
...
def split_folds(X_train, y_train, num_folds):
    ...
    y_lst = np.array_split(y_train, num_folds)
