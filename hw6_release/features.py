...
class PCA(object):
    ...
    def _eigen_decomp(self, X):
        ...
        X_cov = X.T @ X / (N - 1)
        e_vals, e_vecs = np.linalg.eigh(X_cov)
        e_vals = e_vals[::-1]
        e_vecs = e_vecs[:, ::-1]
    ...
    def transform(self, X, n_components):
        ...
        X_proj = X_center @ self.W_pca[:n_components].T
        # END YOUR CODE
    ...
    def reconstruct(self, X_proj):
        ...
        X_center = X_proj @ self.W_pca[:n_components]
...
class LDA(object):
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            X_i = X[y==i]
            mu_i = np.mean(X_i, axis=0, keepdims=True)
            diff_mu = mu_i - mu
            mu_cov = diff_mu.T @ diff_mu * X_i.shape[0]
            scatter_between += mu_cov
            # END YOUR CODE
